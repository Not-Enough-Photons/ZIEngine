// CZBodyAnim::SetTimeScale  --  @ 0x001e1b00 (36 bytes), sizeof=36
// The dsll32 0x1e / dsra32 0x1f pair is how Metrowerks tests one bit.
...
// ----------------------------------------------------------------------
void SetTimeScale(unsigned char* self, float scale)
{
    if (self[0x04] & 0x02)
        scale = ee_mul(scale, *(const float*)(self + 0x10));
    *(float*)(self + 0x0c) = scale;
}
extern "C" unsigned test_entry(unsigned char* o, unsigned a)
{ SetTimeScale(o, (float)a); return 0; }
